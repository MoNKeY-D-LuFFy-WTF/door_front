<template>
    <form @submit="cahngeForm" class="w-100 d-flex flex-column gap-4" :action="`${BACKEND_URL}/order_installations`"
        method="post">
...
export default {
    ...
    methods: {
        cahngeForm(e) {
            ...
            const box = e.target[3].checked;
            if (phone.trim() === '' || name.trim() === '' || !box) {
                e.preventDefault();
            }
        }
    },
